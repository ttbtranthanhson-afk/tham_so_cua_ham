...
function nextStep() {
    ...
    if (activeSlide && activeSlide._pendingCard) {
        ...
        // Summary screen stores a plain object with a remove() that handles transition
        if (card._isSummary) {
            card.remove();
            return;
        }
        // Standard problem card (DOM element)
...
// ==========================================
// Slide 5 Logic — Goodbye
// ==========================================
function handleSlide5(step) {
    // Animate in on first entry
    const tl = gsap.timeline({delay: 0.2});
    tl.fromTo('.bye-circle',  {scale:0, opacity:0},  {scale:1, opacity:1, duration:1.0, ease:'expo.out'})
      .fromTo('#bye-sub',     {opacity:0, y:20},      {opacity:1, y:0, duration:0.5, ease:'power2.out'}, '-=0.4')
      .fromTo('.bye-line1',   {opacity:0, y:40},      {opacity:1, y:0, duration:0.6, ease:'expo.out'}, '-=0.2')
      .fromTo('.bye-line2',   {opacity:0, y:40},      {opacity:1, y:0, duration:0.6, ease:'expo.out'}, '-=0.35')
      .fromTo('#bye-desc',    {opacity:0, y:20},      {opacity:1, y:0, duration:0.5, ease:'power2.out'}, '-=0.2')
      .fromTo('.bye-corner',  {opacity:0, scale:0},   {opacity:1, scale:1, stagger:0.1, duration:0.4, ease:'back.out(2)'}, '-=0.3');

    // Pulse the big circle gently
    gsap.to('.bye-circle', {
        boxShadow: '0 0 120px 40px rgba(100,0,160,0.5)',
        duration: 2.5, yoyo:true, repeat:-1, ease:'sine.inOut', delay:1
    });
    sfx('success');
}
...
const quizData = [
    {
        q: "Trong Python, từ khóa nào dùng để định nghĩa một hàm (chương trình con)?",
        answers: [
    ...
    {
        q: "Hàm prime(n) trong ví dụ 1 trả về True khi nào?",
        answers: [
            { text: "Khi n chia hết cho 2", correct: false },
            { text: "Khi biến đếm C > 1", correct: false },
            { text: "Khi biến đếm C == 1 (chỉ có 1 ước)", correct: true },
            { text: "Khi n là số chẵn", correct: false }
        ]
    },
    {
        q: "Lợi ích nào SAI khi sử dụng chương trình con?",
        answers: [
            { text: "Chia nhỏ bài toán lớn, dễ làm việc nhóm", correct: false },
            { text: "Chương trình chính ngắn gọn, rõ ràng hơn", correct: false },
            { text: "Chương trình con chạy nhanh hơn hàm thường", correct: true },
            { text: "Dễ hiệu chỉnh, phát triển và nâng cấp", correct: false }
        ]
    },
    {
        q: "Trong hàm tongduong(A), câu lệnh 'S = S + k' chỉ được thực hiện khi nào?",
        answers: [
            { text: "Khi k < 0", correct: false },
            { text: "Khi k == 0", correct: false },
            { text: "Khi k >= 0", correct: false },
            { text: "Khi k > 0", correct: true }
        ]
    },
    {
        q: "Để gọi hàm prime(k) kiểm tra từng số từ 1 đến n, chương trình chính dùng cấu trúc nào?",
        answers: [
            { text: "while k < n:", correct: false },
            { text: "for k in range(1, n+1):", correct: true },
            { text: "for k in range(n):", correct: false },
            { text: "if k in range(1, n):", correct: false }
        ]
    }
];
